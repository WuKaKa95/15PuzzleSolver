...
def random_board(N, max_shuffle=None):
    ...
    if max_shuffle is not None:
        ...
        for _ in range(max_shuffle):
            r, c = divmod(zero, N)
            opts = []
            if c > 0:    opts.append(zero-1)
            if c < N-1:  opts.append(zero+1)
            if r > 0:    opts.append(zero-N)
            if r < N-1:  opts.append(zero+N)
            swap = random.choice(opts)
...
if __name__=='__main__':
    ...
    for N, shuf, heus in scenarios:
        ...
        for h in list(data):
            if h=='hamming' and N==4 and shuf is None:
                logging.info("Skipping Hamming on fully-random 4×4")
                del data[h]
                continue
        ...
        plt.xlabel("Path Length"); plt.ylabel("Count"); plt.legend(); plt.grid(True)
        plt.tight_layout(); plt.savefig(f"{outdir}/hist_path_{kind}.png"); plt.close()
